...
test("Windows launches wrapped CLIs through the shell so bare executables resolve", () => {
  const result = normalizeCommandForSpawn("vite", ["--version"], "win32");
  assert.deepEqual(result.command, "cmd.exe");
  assert.equal(result.args[0], "/d");
  assert.equal(result.args[1], "/s");
  assert.equal(result.args[2], "/c");
  assert.equal(result.args[3], "vite");
  assert.deepEqual(result.args.slice(4), ["--version"]);
});
