...
describe("isApprovedEmployeeEmail", () => {
  it("accepts a current company email from the roster", () => {
    assert.equal(isApprovedEmployeeEmail("cuongpk.giong04@gmail.com"), true);
  });

  it("rejects an unapproved personal email", () => {
    assert.equal(isApprovedEmployeeEmail("random.user@gmail.com"), false);
  });
});
